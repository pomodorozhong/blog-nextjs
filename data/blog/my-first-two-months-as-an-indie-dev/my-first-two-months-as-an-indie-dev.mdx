...
☝️And this is what actually happened. Spent two months, and only got the app on the [Testflight](https://developer.apple.com/testflight/). These were some of the things that goes wrong:
...
- The [App Store Connect](https://appstoreconnect.apple.com) blocks me from distributing the app for 2 weeks, because of something that's not my fault. And it takes ages for the support team to even acknowledge the issue.

## The App Store Connect

First, let's address the elephant in the room - [App Store Connect](https://appstoreconnect.apple.com). This is definitely one of the "pain" that I was talking about.
...
[Testflight](https://developer.apple.com/testflight/) is a way to distribute a beta app to people who are willing to test it, it also allows developers to update apps very frequently. It's crucial when we are trying to release a minimum viable product (MVP) and rapidly iterate it based on user feedback.
...
Then I went for Phone support. Surprisingly, that went pretty well. I clicked the button on the website, then they called me in seconds. After a brief talk, they ask me to give them more details by email and promised that this time they will contact me with an email address that I can reply to.

After a few days of back-and-forth email, they notify me that they fixed the Testflight issue. So I proceed to use the same build with the same steps that I had taken to set up Testflight, and it worked.
...
I figured that being an independent developer is just like doing a startup, so ["I'm something of an entrepreneur myself"](https://knowyourmeme.com/memes/im-something-of-a-scientist-myself). And I may learn how to do marketing from those startups.
